...
# loading pre-run seed, init and sims----------------------------------------------------------------------------------
    ...
    def load_config(self):
        ...
        if self.config_name:  # FIXME we need validations that the loaded objects are true to (BETSE config yaml) type!
            ...
            self.config_Fname = paths.get_basename(self.config_name)
            ...
            self.path_to_seed = paths.join(self.p.init_path, self.p.world_filename)
            self.path_to_init = paths.join(self.p.init_path, self.p.init_filename)
            self.path_to_sim = paths.join(self.p.sim_path, self.p.sim_filename)
...
# defining new default BETSE config and associated project directories; saving GUI-modified configs---------------------
    ...
    def new_config(self):
        ...
        if config_dir_name:
            ...
            self.config_name = paths.join(config_dir_name, self.config_Fname)
